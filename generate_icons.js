...
async function generate() {
  ...
  for (const size of sizes) {
    const iconSize = size;
    // We want the logo to be somewhat smaller than the full icon (e.g., 70% of the size) to give it padding
  ...
  .composite([{ 
    input: await sharp(logoPath).resize(358, 358, { fit: 'contain', background: {r:0,g:0,b:0,alpha:0} }).toBuffer(), 
    gravity: 'center' 
  }])
  .png()
  .toBuffer();
